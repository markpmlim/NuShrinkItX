...
// Variables whose name corresponds to those from ShrinkIt
// values from Master header - reserved values are not included.
typedef struct {
...
// Lifted from Andy Fadden's source code
...
-(id) initWithPath:(NSString *)pathToFile {
	...
	if (self != nil) {
		@try {
			dataContents = [NSData dataWithContentsOfFile: pathToFile];
		}
		@catch (NSException * e) {
		...
		if ([dataContents length] < (kMinNuRecordSize + kNuMasterRecordSize)) {
			dataContents = nil;
			items = nil;
			return nil;
		}
		else {
			endOfDataMark = [dataContents length];
			headerOffset = 0;
...
/*
 ...
 For a plain SHK file, the header offset is @ 0x0000.
 ...
 */
	...
	if (currOffset > stopOffset) {
		// Play safe here in case both searches failed.
    ...
    *dataOffset = currOffset;
	...
	if ([signature isEqualToData:signatureSHK]) {
		...
		[self.dataContents getBytes:&inputBuf
							  range:range];
		masterHeaderRecord.total_records = inputBuf[0] + (inputBuf[1] << 8) + (inputBuf[2] << 16) + (inputBuf[3] << 24);
		////NSLog(@"%d", masterHeaderRecord.total_records);
		currOffset += 4;
		range = NSMakeRange(currOffset, 8);
		[self.dataContents getBytes:&masterHeaderRecord.archive_create_when
							  range:range];
...
// Convert IIGS Date and Time to Unix Time
...
-(ShrinkItArchiveItem *) shrinkItItemHeaderAtOffset:(NSUInteger *)dataOffset {
	...
	*dataOffset = currOffset;			// Just in case
	...
	if (signature == 0xD846F54E) {
		...
		item.totalSize = [NSString stringWithFormat:@"%u", total_Size];
...
-(NSArray <ShrinkItArchiveItem *> *) archiveItems {
