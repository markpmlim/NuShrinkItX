...
@interface ShrinkItArchiveItem () {
    ...
    // These instance variables are specific to SpotlightSHK
    NSString *_fileName;
    NSString *_fileType;
    ...
    CompressedDataDescriptor _dataDescriptor;
    CompressedDataDescriptor _resourceDescriptor;
}
@end
...
/*
 @synthesize is no longer needed if we
 Properties are autosynthesized if instance variables are not declared with a preceding _
 in the @interface declaration above.
 */
...
- (instancetype) init {
	self = [super init];
	if (self) {
		_nufile_id = 0xD846F54E;	// Little endian
	}
	return self;
}
