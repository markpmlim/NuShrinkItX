    ...
    NuRecordIdx* pRecordIdx, NuRecord** ppNewRecord)
{
	...
	// added support for a GS/OS option list
	pNewRecord->recOptionSize = pFileDetails->optionSize;
	pNewRecord->recOptionList = pFileDetails->optionList;
/*
	if (pFileDetails->optionSize != 0) {
		pNewRecord->recOptionSize = pFileDetails->optionSize;
		//printf("option size:%d\n", pNewRecord->recOptionSize);
		uint8_t *optionList = malloc(pFileDetails->optionSize);
		//uint8_t *optionList = Nu_Malloc(pArchive, pFileDetails->optionSize);
		memcpy(optionList, pFileDetails->optionList, pFileDetails->optionSize);
		pNewRecord->recOptionList = optionList;
	}
*/
